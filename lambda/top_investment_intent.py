...
TOP_SELECT  = "SELECT {}, investment FROM my_portfolio  "
TOP_JOIN    = " "
TOP_WHERE   = " AND LOWER({}) LIKE LOWER('%{}%') " 
TOP_ORDERBY = " ORDER BY investment desc" 
TOP_DEFAULT_COUNT = '3'
...
def top_intent_handler(intent_request, session_attributes):
    ...
    for dimension in ihbot.DIMENSIONS:
        ...
        logger.debug('<<IHBot>> pre top5_invformatter[%s] = %s', slot_key, slot_values.get(slot_key))
        if slot_values.get(slot_key) is not None:
            ...
            if userexits.DIMENSION_FORMATTERS.get(slot_key) is not None:
                ...
                logger.debug('<<IHBot>> top5_invformatter[%s] = %s', slot_key, output_text)

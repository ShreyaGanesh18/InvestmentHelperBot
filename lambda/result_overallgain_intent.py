...
COUNT_SELECT = "SELECT overall_gain FROM my_portfolio "
COUNT_JOIN = "  "
COUNT_WHERE = " AND LOWER({}) LIKE LOWER('%{}%') "   
COUNT_PHRASE = 'Ovearll Gain'
...
def count_intent_handler(intent_request, session_attributes):
    ...
    for dimension in ihbot.DIMENSIONS:
        ...
        logger.debug('<<IHBot>> pre top5_ognformatter[%s] = %s', slot_key, slot_values.get(slot_key))
        if slot_values.get(slot_key) is not None:
            # the DIMENSION_FORMATTERS perform a formatterpost-process functions and then format the output
            ...
            if userexits.DIMENSION_FORMATTERS.get(slot_key) is not None:
                ...
                logger.debug('<<IHBot>> dimension_ogn[%s] = %s', slot_key, output_text)

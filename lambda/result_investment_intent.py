...
COUNT_SELECT = "SELECT investment FROM my_portfolio "
COUNT_JOIN = "  "
COUNT_WHERE = " AND LOWER({}) LIKE LOWER('%{}%') "   
COUNT_PHRASE = 'Investments so far'
...
def count_intent_handler(intent_request, session_attributes):
    ...
    for dimension in ihbot.DIMENSIONS:
        ...
        logger.debug('<<IHBot>> pre top5_iriformatter[%s] = %s', slot_key, slot_values.get(slot_key))
        if slot_values.get(slot_key) is not None:
            ...
            if userexits.DIMENSION_FORMATTERS.get(slot_key) is not None:
                ...
                logger.debug('<<IHBot>> dimension_iriformatter[%s] = %s', slot_key, output_text)
